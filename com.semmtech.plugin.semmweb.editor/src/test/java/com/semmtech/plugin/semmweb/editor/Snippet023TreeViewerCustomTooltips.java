...
public class Snippet023TreeViewerCustomTooltips {
    ...
    public class MyModel {
        ...
        public ArrayList<Object> child = Lists.newArrayList();
                        ...
                        for (int i = 0; i < columns || i == 0; i++) {
                            if (item.getBounds(i).contains(coords)) {
                                ...
                                tip = new Shell(v.getTree().getShell(), SWT.ON_TOP | SWT.NO_FOCUS
                                        | SWT.TOOL);
                                tip.setBackground(v.getTree().getDisplay()
                                        .getSystemColor(SWT.COLOR_INFO_BACKGROUND));
                                FillLayout layout = new FillLayout();
                                layout.marginWidth = 2;
                                tip.setLayout(layout);
                                label = new Label(tip, SWT.NONE);
                                label.setForeground(v.getTree().getDisplay()
                                        .getSystemColor(SWT.COLOR_INFO_FOREGROUND));
                                label.setBackground(v.getTree().getDisplay()
                                        .getSystemColor(SWT.COLOR_INFO_BACKGROUND));
