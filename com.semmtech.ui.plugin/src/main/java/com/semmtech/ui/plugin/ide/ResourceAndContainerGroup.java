 ...
 * 
 * Copy of org.eclipse.ui.internal.ide.misc.ResourceAndContainerGroup
 */
public class ResourceAndContainerGroup implements Listener {
    ...
    private Combo ontologyFormatCombo;
     ...
     */
    public ResourceAndContainerGroup(Composite parent, Listener client, String resourceFieldLabel,
            String resourceType, boolean showClosedProjects, Map<String, String> formatNames) {
        this(parent, client, resourceFieldLabel, resourceType, showClosedProjects, formatNames,
                SWT.DEFAULT);
    }
    ...
    protected void createContents(Composite parent, String resourceLabelString, int heightHint) {
        ...
        resourceNameField.addFocusListener(new FocusListener() {
            @Override
            public void focusLost(FocusEvent e) {
                updateFileExtension();
            }
        ...
        ontologyFormatCombo = new Combo(nameGroup, SWT.DROP_DOWN | SWT.READ_ONLY);
        String[] items = new String[formatNames.keySet().size()];
        formatNames.keySet().toArray(items);
        ontologyFormatCombo.setItems(items);
        ontologyFormatCombo.select(0);
        ontologyFormatCombo.addModifyListener(new ModifyListener() {
            @Override
    ...
    protected void updateFileExtension() {
        ...
        if (filename.length() > 0) {
            ...
            resourceNameField.setText(filename);
        }
    }

    protected void updateOntologyFormat() {
        String[] extensions = ontologyFormatCombo.getItems();
        String currentFormat = FilenameUtils.getExtension(getResource());
        ...
        for (int i = 0; i < extensions.length; i++) {
            if (extensions[i].contains("." + currentFormat)) {
                ontologyFormatCombo.select(i);
                return;
            }
        }
    }
    ...
    public IContainer getSelectedContainer() {
        return containerGroup.getSelectedContainer();
    }
    ...
    /**
     * Returns the selected ontology format, or serialization language.
     * 
     * @return the selected ontology format, or serialization language.
     */
    public String getExtension() {
        String key = ontologyFormatCombo.getText();
        return formatNames.get(key);
    }
    ...
    private boolean useResourceExtension() {
        ...
        if ((resourceExtension != null) && (resourceExtension.length() > 0)
                && (resource.length() > 0) && (resource.endsWith('.' + resourceExtension) == false)) {
            return true;
        }
        return false;
    }
    ...
    protected void handleOntologyFormatFocusLostEvent() {
    ...
    public void setFocus() {
        containerGroup.treeViewer.getControl().setFocus();
    }
    ...
    protected boolean validateContainer() {
        ...
        if (path == null) {
            ...
            problemMessage = "Resource group folder empty.";
            return false;
        }
        ...
        if (projectName == null || !workspace.getRoot().getProject(projectName).exists()) {
            ...
            problemMessage = "Resource group no project.";
            return false;
        }
    ...
    protected boolean validateFullResourcePath(IPath resourcePath) {
        ...
        if (!allowExistingResources
                && (workspace.getRoot().getFolder(resourcePath).exists() || workspace.getRoot()
                        .getFile(resourcePath).exists())) {
            problemType = PROBLEM_RESOURCE_EXIST;
            // problemMessage = NLS.bind(
            // IDEWorkbenchMessages.ResourceGroup_nameExists, getResource());
            problemMessage = "Resource name exists.";
            return false;
        }
        return true;
    }
    ...
    protected boolean validateResourceName() {
        ...
        if (!Path.ROOT.isValidPath(resourceName)) {
            ...
            // resourceName);
            problemMessage = "Resource name is invalid";
            return false;
        }
        return true;
    }
    ...
    /**
     * Adds a filter to the Project TreeViewer
     */
    public void addViewerFilter(ViewerFilter filter) {
        containerGroup.addViewerFilter(filter);
    }
