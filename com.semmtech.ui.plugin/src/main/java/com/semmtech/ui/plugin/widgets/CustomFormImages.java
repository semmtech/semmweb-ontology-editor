 ...
 * @author Sander Stolk
 */
public class CustomFormImages {
    private static CustomFormImages instance;

    public static CustomFormImages getInstance() {
        if (instance == null)
            instance = new CustomFormImages();
        return instance;
    }

    private ResourceManagerManager manager = new ResourceManagerManager();
    private HashMap<Image, AbstractImageDescriptor> descriptors;

    private CustomFormImages() {
    }
    ...
    private class SimpleImageDescriptor extends AbstractImageDescriptor {
        ...
        SimpleImageDescriptor(Color color1, Color color2, int realtheight, int theight,
                int marginHeight) {
    ...
    private class ComplexImageDescriptor extends AbstractImageDescriptor {
        ...
        public ComplexImageDescriptor(Color[] colors, int length, int[] percents, boolean vertical,
                Color bg) {
            super(colors, length);
        ...
        private void drawTextGradient(GC gc, int width, int height, Color[] colors, int[] percents,
                boolean vertical, Color bg) {
    ...
    public Image getGradient(Color color1, Color color2, int realtheight, int theight,
            int marginHeight, Display display) {
        if (color1 == null || color1.isDisposed() || color2 == null || color2.isDisposed())
            return null;
        AbstractImageDescriptor desc = new SimpleImageDescriptor(color1, color2, realtheight,
                theight, marginHeight);
        return getGradient(desc, display);
    }

    public Image getGradient(Color[] colors, int[] percents, int length, boolean vertical,
            Color bg, Display display) {
        if (colors.length == 0)
            return null;
        for (int i = 0; i < colors.length; i++)
        ...
        if (bg != null && bg.isDisposed())
            return null;
        AbstractImageDescriptor desc = new ComplexImageDescriptor(colors, length, percents,
                vertical, bg);
        return getGradient(desc, display);
    }
    ...
    public synchronized boolean markFinished(Image image, Display display) {
        checkHashMaps();
        AbstractImageDescriptor desc = descriptors.get(image);
        if (desc != null) {
