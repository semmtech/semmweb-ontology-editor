...
package com.semmtech.spin.constraints;
...
public class SPINConstraints {
    ...
    public static void addConstraintViolations(List<ConstraintViolation> results,
            SPINInstance instance, Property predicate, boolean matchValue,
    ...
    /**
     * Creates an RDF representation (instances of spin:ConstraintViolation)
     * from a collection of ConstraintViolation Java objects.
     * 
     * @param cvs
     *            the violation objects
     * @param result
     *            the Model to add the results to
     * @param createSource
     *            true to also create the spin:violationSource
     */
    ...
    private static void addConstructedProblemReports(Model cm, List<ConstraintViolation> results,
            Model model, Resource atClass, Resource matchRoot, String label, Resource source) {
    ...
    private static void addQueryResults(List<ConstraintViolation> results, QueryOrTemplateCall qot,
            Resource resource, boolean matchValue, List<SPINStatistics> stats,
        ...
        if (arq.isAskType()) {
            if (qexec.execAsk() != matchValue) {
                ...
                List<SimplePropertyPath> paths = getPropertyPaths(resource, qot.getQuery()
                        .getWhere(), null);
            ...
            qexec.close();
            addConstructedProblemReports(cm, results, model, qot.getCls(), resource, qot.getQuery()
                    .getComment(), getSource(qot));
        }
        ...
        if (stats != null) {
            ...
            String queryText;
            if (qot.getTemplateCall() != null) {
                queryText = SPINLabels.get()
                        .getLabel(qot.getTemplateCall().getTemplate().getBody());
            }
            else {
    ...
    private static void addTemplateCallResults(List<ConstraintViolation> results,
            QueryOrTemplateCall qot, Resource resource, boolean matchValue, ProgressMonitor monitor) {
        ...
        for (Resource superClass : JenaUtil.getAllSuperClasses(template)) {
            if (JenaUtil.hasIndirectType(superClass, SPIN.Template)) {
                addTemplateCallResults(results, qot, resource, matchValue, templateCall,
                        SPINFactory.asTemplate(superClass));
            }
        }
    }
    ...
    private static void addTemplateCallResults(List<ConstraintViolation> results,
        ...
        if (template != null && template.getBody() instanceof org.topbraid.spin.model.Query) {
            org.topbraid.spin.model.Query spinQuery = (org.topbraid.spin.model.Query) template
                    .getBody();
            if (spinQuery instanceof Ask || spinQuery instanceof Construct) {
                ...
                for (Argument arg : template.getArguments(false)) {
                    if (!arg.isOptional()) {
                        if (!bindings.contains(arg.getVarName())) {
                            // Don't execute this template if any non-optional
                            // argument is missing
                            return;
                        }
                    }
                ...
                if (spinQuery instanceof Ask) {
                    if (qexec.execAsk() != matchValue) {
                        List<SimplePropertyPath> paths = getPropertyPaths(resource,
                                spinQuery.getWhere(), templateCall.getArgumentsMapByProperties());
                        String message = SPINLabels.get().getLabel(templateCall);
                        message += "\n(SPIN constraint at "
                                + SPINLabels.get().getLabel(qot.getCls()) + ")";
                        results.add(createConstraintViolation(paths, NO_FIXES, resource, message,
                                templateCall));
                    }
                }
                    ...
                    addConstructedProblemReports(cm, results, model, qot.getCls(), resource, label,
                            source);
                }
            }
        }
     ...
     * 
     * @param resource
     *            the instance to run constraint checks on
     * @param constraintsModel
     *            the Model that contains the constraints (null if
     *            <code>model</code> should be used for constraintsModel)
     * @param monitor
     *            an (optional) progress monitor (currently ignored)
     * @return a List of ConstraintViolations (empty if all is OK)
     */
    public static List<ConstraintViolation> check(Resource resource, Model constraintsModel,
            ProgressMonitor monitor) {
        return check(resource, constraintsModel, SPIN.constraint, new LinkedList<SPINStatistics>(),
                monitor);
    }
     ...
     *            <code>model</code> should be used for constraintsModel)
     * @param predicate
     *            the system property, e.g. a sub-property of spin:constraint or
     *            null for the default (spin:constraint)
     * @param monitor
     *            an (optional) progress monitor (currently ignored)
     * @return a List of ConstraintViolations (empty if all is OK)
     */
    public static List<ConstraintViolation> check(Resource resource, Model constraintsModel,
        ...
        return check(resource, constraintsModel, predicate, new LinkedList<SPINStatistics>(),
                monitor);
    }
     ...
     *            <code>model</code> should be used for constraintsModel)
     * @param predicate
     *            the system property, i.e. spin:constraint or a sub-property
     *            thereof or null for the default (spin:constraint)
     * @param stats
     *            an (optional) List to add statistics to
     * @param monitor
     *            an (optional) progress monitor (currently ignored)
     * @return a List of ConstraintViolations (empty if all is OK)
     */
    public static List<ConstraintViolation> check(Resource resource, Model constraintsModel,
            Property predicate, List<SPINStatistics> stats, ProgressMonitor monitor) {
        List<ConstraintViolation> results = new LinkedList<>();
        Model model = resource.getModel();

        OntModel spinModel = null;
        OntModel unionModel = null;

        if (constraintsModel == null) {
            spinModel = SPINImports.createSPINModel(model);
            unionModel = spinModel;
        }
        else {
            spinModel = SPINImports.createSPINModel(constraintsModel);

            unionModel = ModelFactory.createOntologyModel(OntModelSpec.RDFS_MEM);
            unionModel.addSubModel(model);
            unionModel.addSubModel(spinModel);
        }

        resource = resource.inModel(unionModel);
     ...
     * returns a List of constraint violations. A ProgressMonitor can be
     * provided to enable the user to get intermediate status reports and to
     ...
     */
    public static List<ConstraintViolation> check(Model model, Model constraintsModel,
            ProgressMonitor monitor) {
        return check(model, constraintsModel, SPIN.constraint, null, monitor);
    }
    ...
    public static List<ConstraintViolation> check(Model model, Model constraintsModel,
            Property predicate, ProgressMonitor monitor) {
        return check(model, constraintsModel, predicate, null, monitor);
    }
     ...
     *            <code>model</code> should be used for constraintsModel)
     * @param predicate
     *            the system property, e.g. a sub-property of spin:constraint
     * @param stats
     *            an (optional) List to write statistics reports to
     * @param monitor
     *            an optional ProgressMonitor
     * @return a List of ConstraintViolations
     */
    public static List<ConstraintViolation> check(Model model, Model constraintsModel,
            Property predicate, List<SPINStatistics> stats, ProgressMonitor monitor) {
        List<ConstraintViolation> results = new LinkedList<>();
        run(model, constraintsModel, predicate, results, stats, monitor);
        return results;
    }

    private static synchronized Query convertAskToConstruct(Query ask,
        ...
        try {
            ...
            if (label != null) {
                bgp.add(Triple.create(subject, RDFS.label.asNode(),
                        NodeFactory.createLiteral(label)));
            }
            com.hp.hpl.jena.sparql.syntax.Template template = new com.hp.hpl.jena.sparql.syntax.Template(
                    bgp);
    ...
    private static ConstraintViolation createConstraintViolation(
            Collection<SimplePropertyPath> paths, Collection<TemplateCall> fixes,
            Resource instance, String message, Resource source) {
        return new ConstraintViolation(instance, paths, fixes, message, source);
    }

    private static QuerySolutionMap createInitialBindings(Resource resource,
    ...
    private static List<TemplateCall> getFixes(Model cm, Model model, Resource vio) {
        ...
        while (fit.hasNext()) {
            ...
            if (fs.getObject().isResource()) {
                MultiUnion union = JenaUtil.createMultiUnion(new Graph[] { model.getGraph(),
                        cm.getGraph() });
    ...
    private static List<SimplePropertyPath> getPropertyPaths(Resource resource, ElementList where,
            Map<Property, RDFNode> varBindings) {
        if (where != null) {
            PropertyPathsGetter getter = new PropertyPathsGetter(where, varBindings);
            getter.run();
            return new ArrayList<>(getter.getResults());
        }
    ...
    private static List<SimplePropertyPath> getViolationPaths(Model model, Resource vio,
            Resource root) {
    ...
    /**
     * Checks if a given property is a SPIN constraint property. This is defined
     * as a property that is spin:constraint or a sub-property of it.
     * 
     * @param property
    ...
    public static boolean isConstraintProperty(Property property) {
        ...
        else if (JenaUtil.hasSuperProperty(property,
                property.getModel().getProperty(SPIN.constraint.getURI()))) {
            return true;
        }
        else {
            return false;
    ...
    private static void run(Model model, Model constraintsModel, Property predicate,
            List<ConstraintViolation> results, List<SPINStatistics> stats, ProgressMonitor monitor) {
        ...
        if (constraintsModel == null) {
            ...
            unionModel = ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM);
            unionModel.addSubModel(model);
            unionModel.addSubModel(spinModel);
        }
        ...
        Map<Resource, List<CommandWrapper>> class2Query = SPINQueryFinder.getClass2QueryMap(
                spinModel, spinModel, predicate, true, true);
        ...
        if (monitor != null) {
            ...
            monitor.beginTask("Checking SPIN Constraints on " + class2Query.size() + " classes",
                    totalWork);
        }
        for (Resource cls : class2Query.keySet()) {
            List<CommandWrapper> arqs = class2Query.get(cls);
            for (CommandWrapper arqWrapper : arqs) {
                ...
                runQueryOnClass(results, arq, queryWrapper.getSPINQuery(), label, model,
                        unionModel, cls, queryWrapper.getTemplateBinding(),
                        arqWrapper.isThisUnbound(), arqWrapper.isThisDeep(),
                        arqWrapper.getSource(), stats, monitor);
                if (!arqWrapper.isThisUnbound()) {
                    Set<Resource> subClasses = JenaUtil.getAllSubClasses(cls);
                    for (Resource subClass : subClasses) {
                        runQueryOnClass(results, arq, queryWrapper.getSPINQuery(), label, model,
                                unionModel, subClass, queryWrapper.getTemplateBinding(),
                                arqWrapper.isThisUnbound(), arqWrapper.isThisDeep(),
                                arqWrapper.getSource(), stats, monitor);
                    }
                }
    ...
    private static void runQueryOnClass(List<ConstraintViolation> results, Query arq,
            org.topbraid.spin.model.Query spinQuery, String label, Model instancesModel,
            Model combinedModel, Resource cls, Map<String, RDFNode> initialBindings,
            boolean thisUnbound, boolean thisDeep, Resource source, List<SPINStatistics> stats,
            ProgressMonitor monitor) {
        if (thisUnbound || SPINUtil.isRootClass(cls)
                || instancesModel.contains(null, RDF.type, cls)) {
            QuerySolutionMap arqBindings = new QuerySolutionMap();
            if (!thisUnbound) {
            ...
            if (thisDeep && !thisUnbound) {
                StmtIterator it = instancesModel.listStatements(null, RDF.type, cls);
                while (it.hasNext()) {
                    ...
                    QueryExecution qexec = ARQFactory.get().createQueryExecution(arq,
                            instancesModel, arqBindings);
                    qexec.execConstruct(cm);
                    qexec.close();
                }
            }
            else {
                QueryExecution qexec = ARQFactory.get().createQueryExecution(arq, instancesModel,
                        arqBindings);
                qexec.execConstruct(cm);
                qexec.close();
            }
            ...
            addConstructedProblemReports(cm, results, combinedModel, cls, null, label, source);
        }
    }
}
