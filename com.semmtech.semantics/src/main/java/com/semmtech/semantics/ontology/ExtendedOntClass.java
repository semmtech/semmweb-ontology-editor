...
public class ExtendedOntClass extends ExtendedOntResource implements OntClass {

    // Constants
    // ////////////////////////////////

    /* LDP never returns properties in these namespaces */
    private static final String[] IGNORE_NAMESPACES = new String[] { OWL.NS, RDF.getURI(),
            RDFS.getURI(), ReasonerVocabulary.RBNamespace };
    ...
    public static Implementation factory = new Implementation() {
        ...
        public EnhNode wrap(Node n, EnhGraph eg) {
            if (canWrap(n, eg)) {
                return new ExtendedOntClass(n, eg);
            }
    ...
    /**
     * <p>
     * Construct an ontology class node represented by the given node in the
     * given graph.
     * </p>
     * 
     * @param n
     ...
     */
    public ExtendedOntClass(Node n, EnhGraph g) {
        super(n, g);
    }

    // External signature methods different from OntClassImpl.class
    // ////////////////////////////////

    /**
     * <p>
     * Answer an iterator over all of the classes that are declared to be
     * super-classes of this class. Each element of the iterator will be an
     * {@link OntClass}. See {@link #listSubClasses(boolean )} for a full
     * explanation of the <em>direct</em> parameter.
     * </p>
     * 
     * @param direct
     *            If true, only answer the directly adjacent classes in the
     *            super-class relation: i&#046;e&#046; eliminate any class for
     *            which there is a longer route to reach that child under the
     *            super-class relation.
     * @return an iterator over the resources representing this class's
     *         sub-classes.
     * @exception OntProfileException
     *                If the {@link Profile#SUB_CLASS_OF()} property is not
    ...
    public ExtendedIterator<OntClass> listSuperClasses(boolean direct) {
        if (direct && workingOnInfGraph()) {
            throw new NotImplementedException(
                    "Cannot yet list direct super classes within an inferred graph.");
        }
        ...
        QueryBuilder qb = QueryBuilder.createSelect(true).addResultVar(varS);
        addListSuperClassPatterns(qb, varS, direct);

        return createOntClassIterator(model, qb.execSelect(model), varS);
    }

    private void addListSuperClassPatterns(QueryBuilder qb, Node superClassNode, boolean direct) {
        if (direct) {
            Triple pattern = Triple.create(asNode(), RDFS.subClassOf.asNode(), superClassNode);
            Triple filter = Triple.create(asNode(),
                    PathUtil.getNode(PathUtil.INDIRECT_SUBCLASS_OF), superClassNode);
            qb.addTriplePattern(pattern).addFilterPattern(filter, true);
        }
        else {
            Triple pattern = Triple.create(asNode(),
                    PathUtil.getNode(PathUtil.INFERRED_SUBCLASS_OF), superClassNode);
            qb.addTriplePattern(pattern);
        }
    }
     ...
     * {@link #listSubClasses(boolean )} for a full explanation of the
     * <em>direct</em> parameter. Note that, unlike listSuperClasses(), this
     * function considers a class to be an (indirect) super class of itself.
     * </p>
     * 
     * @param cls
     *            A class to test.
     * @param direct
     *            If true, only search the classes that are directly adjacent to
     *            this class in the class hierarchy.
    ...
    public boolean hasSuperClass(Resource cls, boolean direct) {
        ...
        addListSuperClassPatterns(qb, cls.asNode(), direct);
        return qb.execAsk(model);
    }

    /**
     * <p>
     * Answer an iterator over the classes that are declared to be sub-classes
     * of this class. Each element of the iterator will be an {@link OntClass}.
     * The distinguishing extra parameter for this method is the flag
     * <code>direct</code> that allows some selectivity over the classes that
     * appear in the iterator. Consider the following scenario: <code><pre>
     *   :B rdfs:subClassOf :A.
     *   :C rdfs:subClassOf :A.
     *   :D rdfs:subClassof :C.
     * </pre></code> (so A has two sub-classes, B and C, and C has sub-class D).
     * In a raw model, with no inference support, listing the sub-classes of A
     * will answer B and C. In an inferencing model,
     * <code>rdfs:subClassOf</code> is known to be transitive, so the
     * sub-classes iterator will include D. The <code>direct</code> sub-classes
     * are those members of the closure of the subClassOf relation, restricted
     * to classes that cannot be reached by a longer route, i.e. the ones that
     * are <em>directly</em> adjacent to the given root. Thus, the direct
     * sub-classes of A are B and C only, and not D - even in an inferencing
     * graph. Note that this is not the same as the entailments from the raw
     * graph. Suppose we add to this example: <code><pre>
     *   :D rdfs:subClassof :A.
     * </pre></code> Now, in the raw graph, A has sub-class C. But the direct
     * sub-classes of A remain B and C, since there is a longer path A-C-D that
     * means that D is not a direct sub-class of A. The assertion in the raw
     * graph that A has sub-class D is essentially redundant, since this can be
     * inferred from the closure of the graph.
     * </p>
     * <p>
     * <strong>Note:</strong> This is is a change from the behaviour of Jena 1,
     * which took a parameter <code>closed</code> to compute the closure over
     * transitivity and equivalence of sub-classes. The closure capability in
     * Jena2 is determined by the inference engine that is wrapped with the
     * ontology model. The direct parameter is provided to allow, for example, a
     * level-by-level traversal of the class hierarchy, starting at some given
     * root.
     * </p>
     * 
     * @param direct
     *            If true, only answer the directly adjacent classes in the
     *            sub-class relation: i&#046;e&#046; eliminate any class for
     *            which there is a longer route to reach that child under the
     *            sub-class relation.
     * @return an iterator over the resources representing this class's
     *         sub-classes
    ...
    public ExtendedIterator<OntClass> listSubClasses(boolean direct) {
        ...
        QueryBuilder qb = QueryBuilder.createSelect(true).addResultVar(varS);
        addListSubClassPatterns(qb, varS, direct);

        return createOntClassIterator(model, qb.execSelect(model), varS);
    }

    private void addListSubClassPatterns(QueryBuilder qb, Node subClassNode, boolean direct) {
        if (direct) {
            Triple pattern = Triple.create(subClassNode, RDFS.subClassOf.asNode(), asNode());
            Triple filter = Triple.create(subClassNode,
                    PathUtil.getNode(PathUtil.INDIRECT_SUBCLASS_OF), asNode());
            qb.addTriplePattern(pattern).addFilterPattern(filter, true);
        }
        else {
            Triple pattern = Triple.create(subClassNode,
                    PathUtil.getNode(PathUtil.INFERRED_SUBCLASS_OF), asNode());
            qb.addTriplePattern(pattern);
        }
    }
     ...
     * {@link #listSubClasses(boolean )} for a full explanation of the
     * <em>direct</em> parameter. Note that, unlike listSubClasses(), this
     * function considers a class to be an indirect sub class of itself.
     * </p>
     * 
     * @param cls
     *            A class to test.
    ...
    public boolean hasSubClass(Resource cls, boolean direct) {
        ...
        // Create query
        QueryBuilder qb = QueryBuilder.createAsk();
        addListSubClassPatterns(qb, cls.asNode(), direct);
        return qb.execAsk(model);
    }

    /**
     * <p>
     * Answer an iterator over the individuals in the model that have this class
     * among their types, optionally excluding sub-classes of this class.
     * <p>
     * 
     * @param direct
    ...
    public ExtendedIterator<Individual> listInstances(final boolean direct) {
        ...
        // Create pattern
        Var varI = Var.alloc("instance");
        Node predicate = (direct) ? RDF.type.asNode() : PathUtil.getNode(PathUtil.IS_INSTANCE_OF);
        Triple pattern = Triple.create(varI, predicate, asNode());

        // Create query
        QueryBuilder qb = QueryBuilder.createSelect(true).addResultVar(varI);
        qb.addTriplePattern(pattern);

        return createIndividualIterator(model, qb.execSelect(model), varI);
    }
     ...
     * statements for <code>subClassOf</code> will be removed.
     * </p>
     * 
     * @param cls
     ...
     * than one such class, an arbitrary selection is made.
     * </p>
     * 
    ...
    /**
     * <p>
     * Answer true if the given class is a super-class of this class. Note that,
     * unlike listSuperClasses(), this function considers a class to be an
     * indirect super class of itself.
     * </p>
     * 
     * @param cls
     *            A class to test.
    ...
    /**
     * <p>
     * Answer true if this class has any direct super-class in the model. Note
     * that when using a reasoner, all OWL classes have owl:Thing as a
     * super-class.
     * </p>
     * 
    ...
    /**
     * <p>
     * Remove the given class from the super-classes of this class. If this
     * statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param cls
     *            A class to be removed from the super-classes of this class
     * @exception OntProfileException
     *                If the {@link Profile#SUB_CLASS_OF()} class is not
     *                supported in the current language profile.
     */
    @Override
     ...
     * statements for <code>subClassOf</code> on <code>prop</code> will be
     * removed.
     * </p>
     * 
     * @param cls
    ...
    public void setSubClass(Resource cls) {
        ...
        for (StmtIterator i = getModel().listStatements(null, getProfile().SUB_CLASS_OF(), this); i
                .hasNext();) {
            i.removeNext();
        }
    ...
    /**
     * <p>
     * Answer a class that is the sub-class of this class. If there is more than
     * one such class, an arbitrary selection is made. If there is no such
     * class, return null.
     * </p>
     * 
     ...
     * <p>
     * Answer an iterator over all of the classes that are declared to be
     * sub-classes of this class. Each element of the iterator will be an
     * {@link OntClass}.
     * </p>
     * 
    ...
    /**
     * <p>
     * Answer true if the given class is a sub-class of this class. Note that,
     * unlike listSubClasses(), this function considers a class to be an
     * indirect sub class of itself.
     * </p>
     * 
     * @param cls
     *            A class to test.
    ...
    /**
     * <p>
     * Answer true if this class has any direct sub-class in the model. Note
     * that when using a reasoner, all OWL classes have owl:Nothing as a
     * sub-class.
     * </p>
     * 
    ...
    /**
     * <p>
     * Remove the given class from the sub-classes of this class. If this
     * statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param cls
     ...
     * statements for <code>equivalentClass</code> will be removed.
     * </p>
     * 
     * @param cls
     *            The class that this class is a equivalent to.
     * @exception OntProfileException
     *                If the {@link Profile#EQUIVALENT_CLASS()} property is not
     *                supported in the current language profile.
     */
    @Override
    ...
    /**
     * <p>
     * Answer a class that is equivalent to this class. If there is more than
     * one such class, an arbitrary selection is made.
     * </p>
     * 
     ...
     * <p>
     * Answer an iterator over all of the classes that are declared to be
     * equivalent classes to this class. Each element of the iterator will be an
     * {@link OntClass}.
     * </p>
     * 
     ...
     * If this statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param cls
     *            A class that may be declared to be equivalent to this class,
     *            and which is no longer equivalent
     * @exception OntProfileException
     *                If the {@link Profile#EQUIVALENT_CLASS()()} property is
     ...
     * statements for <code>disjointWith</code> will be removed.
     * </p>
     * 
     * @param cls
     *            The property that this class is disjoint with.
     * @exception OntProfileException
     *                If the {@link Profile#DISJOINT_WITH()} property is not
     *                supported in the current language profile.
     */
    @Override
    ...
    /**
     * <p>
     * Answer a class with which this class is disjoint. If there is more than
     * one such class, an arbitrary selection is made.
     * </p>
     * 
    ...
    /**
     * <p>
     * Answer an iterator over all of the classes that this class is declared to
     * be disjoint with. Each element of the iterator will be an
     * {@link OntClass}.
     * </p>
     * 
    ...
    public ExtendedIterator<OntClass> listDisjointWith() {
        return listAs(getProfile().DISJOINT_WITH(), "DISJOINT_WITH", OntClass.class).filterKeep(
                new UniqueFilter<OntClass>());
    }
    ...
    /**
     * <p>
     * Remove the statement that this class and the given class are disjoint. If
     * this statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param cls
     *            A class that may be declared to be disjoint with this class,
     *            and which is no longer disjoint
     * @exception OntProfileException
     *                If the {@link Profile#DISJOINT_WITH()()()} property is not
     *                supported in the current language profile.
     */
    ...
    /**
     * <p>
     * Return an iterator over the properties associated with a frame-like view
     * of this class. This captures an intuitive notion of the <em>
     ...
     * properties in this class's {@link OntModel} with the class itself. See:
     * <a href="/documentation/notes/rdf-frames.html">Presenting RDF as
     * frames</a> for more details.
     * </p>
     * <p>
     * Note that many cases of determining whether a property is associated with
     * a class depends on RDFS or OWL reasoning. This method may therefore
     * return complete results only in models that have an attached reasoner.
     * </p>
     * 
     * @param direct
     *            If <code>true</code>, restrict the properties returned to
     *            those directly associated with this class. If
     *            <code>false</code>, the properties of super-classes of this
     *            class will not be listed among the declared properties of this
     *            class.
     * @return An iteration of the properties that are associated with this
     *         class by their domain.
     */
    @Override
    public ExtendedIterator<OntProperty> listDeclaredProperties(boolean direct) {
        // first collect the candidate properties
        Set<RDFNode> candSet = Sets.newHashSet();
        ...
        // domain constraints
        List<RDFNode> cands = Lists.newArrayList();
    ...
    /**
     * <p>
     * Answer true if the given property is one of the declared properties of
     * this class. For details, see {@link #listDeclaredProperties(boolean)}.
     * </p>
     * 
     * @param p
     *            A property to test
     * @param direct
     *            If true, only direct associations between classes and
     *            properties are considered
     * @return True if <code>p</code> is one of the declared properties of this
     *         class
     */
    @Override
     ...
     * {@link Individual#removeOntClass} method, but invoked via the class
     * resource rather than via the individual resource.
     * </p>
     * 
     * @param individual
     *            A resource denoting an individual that is no longer to be a
     ...
     * This will be true if either (i) this class has <code>owl:Thing</code> as
     * a direct super-class, or (ii) it has no declared super-classes (including
    ...
    public boolean isHierarchyRoot() {
        ...
        /**
         * Note: moved the initialisation of i outside the try-catch, otherwise
         * an exception in listSuperClasses [eg a broken Graph implementation]
         * will avoid i's initialisation but still run i.close, generating a
         * mysterious NullPointerException. Signed, Mr Burnt Spines.
         */
        ...
        try {
            ...
            while (i.hasNext()) {
                Resource sup = i.next();
                if (!(sup.equals(getProfile().THING()) || sup.equals(RDFS.Resource) || sup
                        .equals(this))) {
     ...
     *                given the lanuage profile and the current state of the
     *                underlying model.
     */
    @Override
     ...
     * @exception ConversionException
     *                if the class cannot be converted to a union class given
     *                the lanuage profile and the current state of the
     *                underlying model.
     */
    @Override
     ...
     * @exception ConversionException
     *                if the class cannot be converted to a restriction given
     *                the lanuage profile and the current state of the
     *                underlying model.
     */
    @Override
     ...
     * 
     * @param individuals
     *            A list of the individuals that will comprise the permitted
     *            values of this class converted to an enumeration
     * @return This ontology class, converted to an enumeration of the given
     *         individuals
     */
    @Override
     ...
     * 
     * @param classes
     *            A list of the classes that will comprise the operands of the
     *            intersection
     * @return This ontology class, converted to an intersection of the given
     *         classes
     */
    @Override
    public IntersectionClass convertToIntersectionClass(RDFList classes) {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // setPropertyValue(getProfile().INTERSECTION_OF(), "INTERSECTION_OF",
        // classes);
        // return as(IntersectionClass.class);
    }
     ...
     * @return This ontology class, converted to an complement of the given
     *         class
     */
    @Override
     ...
     * 
     * @param prop
     *            A property this is the subject of a property restriction class
     *            expression
     * @return This ontology class, converted to a restriction on the given
     *         property
     */
    @Override
    ...
    /**
     * <p>
     * Answer true if this class has the given class as a direct super-class,
     * without using extra help from the reasoner.
     * </p>
     * 
     * @param cls
     *            The class to test
    ...
    protected boolean hasSuperClassDirect(Resource cls) {
        ...
        ExtendedIterator<OntClass> i = listDirectPropertyValues(getProfile().SUB_CLASS_OF(),
                "subClassOf", OntClass.class, getProfile().SUB_CLASS_OF(), true, false);
        try {
            while (i.hasNext()) {
    ...
    protected boolean testDomain(Property p, boolean direct) {
        ...
        for (StmtIterator i = getModel().listStatements(p, getProfile().DOMAIN(), (RDFNode) null); i
                .hasNext();) {
    ...
    protected ExtendedIterator<Statement> listAllProperties() {
        ...
        ExtendedIterator<Statement> pi = mOnt.listStatements(null, RDF.type, getProfile()
                .PROPERTY());
    ...
    /**
     * <p>
     * Answer true if we can demonstrate that this class has the given
     * super-class. If this model has a reasoner, this is equivalent to asking
     * if the sub-class relation holds. Otherwise, we simulate basic reasoning
     * by searching upwards through the class hierarchy.
     * </p>
     * 
     * @param sup
    ...
    protected boolean canProveSuperClass(Resource sup) {
        ...
        if (om.getReasoner() != null) {
            if (om.getReasoner().getReasonerCapabilities()
                    .contains(null, ReasonerVocabulary.supportsP, RDFS.subClassOf)) {
        ...
        Set<OntClass> seen = Sets.newHashSet();
        List<OntClass> queue = Lists.newArrayList();
        queue.add(this);
    ...
    protected static ExtendedIterator<OntClass> createOntClassIterator(OntModel model,
    ...
    protected static ExtendedIterator<Individual> createIndividualIterator(OntModel model,
            ResultSet iter, Var var) {
        String varName = var.getName();
        List<Individual> individuals = Lists.newArrayList();
        while (iter.hasNext()) {
            QuerySolution querySolution = iter.next();
            Resource individual = querySolution.getResource(varName);
            individuals.add(JenaUtil.asIndividual(individual, model));
        }
