...
public class ExtendedOntProperty extends ExtendedOntResource implements OntProperty {
    // Static variables
    // ////////////////////////////////

    /**
     * A factory for generating OntProperty facets from nodes in enhanced
    ...
    public static Implementation factory = new Implementation() {
        ...
        public EnhNode wrap(Node n, EnhGraph eg) {
            if (canWrap(n, eg)) {
                return new ExtendedOntProperty(n, eg);
            }
    ...
    public ExtendedOntProperty(Node n, EnhGraph g) {
        super(n, g);
    }

    // External signature methods different from OntPropertyImpl.class
    // ////////////////////////////////

    /**
     * <p>
     * Answer an iterator over all of the properties that are declared to be
     * super-properties of this property. Each element of the iterator will be
     * an {@link OntProperty}.
     * </p>
     * 
     * @param direct
     *            If true, only answer the directly adjacent properties in the
     *            property hierarchy: i&#046;e&#046; eliminate any property for
     ...
     *                If the {@link Profile#SUB_PROPERTY_OF()} property is not
    ...
    public ExtendedIterator<OntProperty> listSuperProperties(boolean direct) {
        if (direct && workingOnInfGraph()) {
            throw new NotImplementedException(
                    "Cannot yet list direct super properties within an inferred graph.");
        }

        OntModel model = getOntModel();

        String varName = "superProperty";
        Var varS = Var.alloc(varName);
        QueryBuilder qb = QueryBuilder.createSelect(true).addResultVar(varS);
        addListSuperPropertiesPatterns(qb, varS, direct);

        List<OntProperty> result = Lists.newArrayList();
        for (ResultSet iter = qb.execSelect(model); iter.hasNext();) {
            Resource resource = iter.next().getResource(varName);
            result.add(JenaUtil.asOntProperty(resource, model));
        }
        return UniqueExtendedIterator.create(result.iterator());
    }

    private void addListSuperPropertiesPatterns(QueryBuilder qb, Node superPropertyNode,
            boolean direct) {
        if (direct) {
            Triple pattern = Triple
                    .create(asNode(), RDFS.subPropertyOf.asNode(), superPropertyNode);
            Triple filter = Triple.create(asNode(),
                    PathUtil.getNode(PathUtil.INDIRECT_SUBPROPERTY_OF), superPropertyNode);
            qb.addTriplePattern(pattern).addFilterPattern(filter, true);
        }
        else {
            Triple pattern = Triple.create(asNode(),
                    PathUtil.getNode(PathUtil.INFERRED_SUBPROPERTY_OF), superPropertyNode);
     ...
     * @param direct
     *            If true, only answer the directly adjacent properties in the
     *            property hierarchy: i.e. eliminate any property for which
     *            there is a longer route to reach that child under the
     *            sub-property relation. If false, the returned iterator also
     *            lists the calling property itself as one of its sub
     *            properties.
    ...
    public ExtendedIterator<OntProperty> listSubProperties(boolean direct) {
        if (direct && workingOnInfGraph()) {
            throw new NotImplementedException(
                    "Cannot yet list direct sub properties within an inferred graph.");
        }

        OntModel model = getOntModel();

        // Create query
        String varName = "subProperty";
        Var varS = Var.alloc(varName);
        QueryBuilder qb = QueryBuilder.createSelect(true).addResultVar(varS);
        addListSubPropertiesPatterns(qb, varS, direct);
    ...
    private void addListSubPropertiesPatterns(QueryBuilder qb, Node subPropertyNode, boolean direct) {
        if (direct) {
            Triple pattern = Triple.create(subPropertyNode, RDFS.subPropertyOf.asNode(), asNode());
            Triple filter = Triple.create(subPropertyNode,
                    PathUtil.getNode(PathUtil.INDIRECT_SUBPROPERTY_OF), asNode());
            qb.addTriplePattern(pattern).addFilterPattern(filter, true);
        }
        else {
            Triple pattern = Triple.create(subPropertyNode,
                    PathUtil.getNode(PathUtil.INFERRED_SUBPROPERTY_OF), asNode());
            qb.addTriplePattern(pattern);
        }
    }
     ...
     * Note that, unlike with listSuperProperties(), this function considers the
     * calling property an indirect super property of itself.
     * </p>
     * 
     * @param prop
     *            A property to test.
    ...
    public boolean hasSuperProperty(Property prop, boolean direct) {
        ...
        addListSuperPropertiesPatterns(qb, prop.asNode(), direct);
     ...
     * Note that, unlike with listSubProperties(), this function considers the
     * calling property an indirect super property of itself.
     * </p>
     * 
     * @param prop
    ...
    public boolean hasSubProperty(Property prop, boolean direct) {
        ...
        QueryBuilder qb = QueryBuilder.createAsk();
        addListSubPropertiesPatterns(qb, prop.asNode(), direct);
        return qb.execAsk(model);
    }

    // External signature methods copied from OntPropertyImpl.class
    // ////////////////////////////////
    ...
    /**
     * <p>
     * Assert that this property is sub-property of the given property. Any
     * existing statements for <code>subPropertyOf</code> will be removed.
     * </p>
     * 
     * @param prop
    ...
    /**
     * <p>
     * Answer a property that is the super-property of this property. If there
     * is more than one such property, an arbitrary selection is made.
     * </p>
     * 
    ...
    /**
     * <p>
     * Remove the given property from the super-properties of this property. If
     * this statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param prop
     *            A property to be removed from the super-properties of this
     *            property
     * @exception OntProfileException
     *                If the {@link Profile#SUB_PROPERTY_OF()} property is not
     *                supported in the current language profile.
    ...
    /**
     * <p>
     * Assert that this property is super-property of the given property. Any
     * existing statements for <code>subPropertyOf</code> on <code>prop</code>
     * will be removed.
     * </p>
     * 
     * @param prop
    ...
    public void setSubProperty(Property prop) {
        ...
        for (StmtIterator i = getModel().listStatements(null, getProfile().SUB_PROPERTY_OF(), this); i
                .hasNext();) {
            i.removeNext();
        }
    ...
    public OntProperty getSubProperty() {
        ...
        return getModel().listStatements(null, getProfile().SUB_PROPERTY_OF(), this)
                .nextStatement().getSubject().as(OntProperty.class);
    }
    ...
    /**
     * <p>
     * Remove the given property from the sub-properties of this property. If
     * this statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param prop
     *            A property to be removed from the sub-properties of this
     *            property
     * @exception OntProfileException
     *                If the {@link Profile#SUB_PROPERTY_OF()} property is not
     *                supported in the current language profile.
    ...
    /**
     * <p>
     * Assert that the given resource represents the class of individuals that
     * form the domain of this property. Any existing <code>domain</code>
     * statements for this property are removed.
     * </p>
     * 
     * @param res
     *            The resource that represents the domain class for this
     *            property.
     * @exception OntProfileException
     *                If the {@link Profile#DOMAIN()} property is not supported
     *                in the current language profile.
     */
    @Override
    ...
    /**
     * <p>
     * Answer a resource that represents the domain class of this property. If
     * there is more than one such resource, an arbitrary selection is made.
     * </p>
     * 
     * @return An resource representing the class that forms the domain of this
     *         property
     * @exception OntProfileException
     *                If the {@link Profile#DOMAIN()} property is not supported
     *                in the current language profile.
    ...
    /**
     * <p>
     * Answer an iterator over all of the declared domain classes of this
     * property. Each element of the iterator will be an {@link OntResource}.
     * </p>
     * 
     * @return An iterator over the classes that form the domain of this
     *         property.
     * @exception OntProfileException
     *                If the {@link Profile#DOMAIN()} property is not supported
     *                in the current language profile.
    ...
    /**
     * <p>
     * Answer true if the given resource a class specifying the domain of this
     * property.
     * </p>
     * 
     * @param res
     *            A resource representing a class
     * @return True if the given resource is one of the domain classes of this
     *         property.
     */
    @Override
    ...
    /**
     * <p>
     * Remove the given class from the stated domain(s) of this property. If
     * this statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param cls
     *            A class to be removed from the declared domain(s) of this
     *            property
     * @exception OntProfileException
     *                If the {@link Profile#DOMAIN()} property is not supported
     *                in the current language profile.
     ...
     * <p>
     * Assert that the given resource represents the class of individuals that
     * form the range of this property. Any existing <code>range</code>
     * statements for this property are removed.
     * </p>
     * 
     * @param res
     *            The resource that represents the range class for this
     *            property.
     * @exception OntProfileException
     *                If the {@link Profile#RANGE()} property is not supported
     *                in the current language profile.
     */
    @Override
    ...
    /**
     * <p>
     * Answer a resource that represents the range class of this property. If
     * there is more than one such resource, an arbitrary selection is made.
     * </p>
     * 
     * @return An resource representing the class that forms the range of this
     *         property
     * @exception OntProfileException
     *                If the {@link Profile#RANGE()} property is not supported
     *                in the current language profile.
    ...
    /**
     * <p>
     * Answer an iterator over all of the declared range classes of this
     * property. Each element of the iterator will be an {@link OntResource}.
     * </p>
     * 
     * @return An iterator over the classes that form the range of this
     *         property.
     * @exception OntProfileException
     *                If the {@link Profile#RANGE()} property is not supported
     *                in the current language profile.
    ...
    /**
     * <p>
     * Answer true if the given resource a class specifying the range of this
     * property.
     * </p>
     * 
     * @param res
     *            A resource representing a class
     * @return True if the given resource is one of the range classes of this
     *         property.
     */
    @Override
    ...
    /**
     * <p>
     * Remove the given class from the stated range(s) of this property. If this
     * statement is not true of the current model, nothing happens.
     * </p>
     * 
     * @param cls
     *            A class to be removed from the declared range(s) of this
     *            property
     * @exception OntProfileException
     *                If the {@link Profile#RANGE()} property is not supported
     *                in the current language profile.
    ...
    /**
     * <p>
     * Assert that the given property is equivalent to this property. Any
     * existing statements for <code>equivalentProperty</code> will be removed.
     * </p>
     * 
     * @param prop
     *            The property that this property is a equivalent to.
     * @exception OntProfileException
     *                If the {@link Profile#EQUIVALENT_PROPERTY()} property is
    ...
    /**
     * <p>
     * Answer a property that is equivalent to this property. If there is more
     * than one such property, an arbitrary selection is made.
     * </p>
     * 
     ...
     * <p>
     * Answer an iterator over all of the properties that are declared to be
     * equivalent properties to this property. Each element of the iterator will
     * be an {@link OntProperty}.
     * </p>
     * 
     ...
     * equivalent. If this statement is not true of the current model, nothing
     * happens.
     * </p>
     * 
     * @param prop
     *            A property that may be declared to be equivalent to this
     *            property
     * @exception OntProfileException
     *                If the {@link Profile#EQUIVALENT_PROPERTY()} property is
     *                not supported in the current language profile.
    ...
    /**
     * <p>
     * Assert that the given property is the inverse of this property. Any
     * existing statements for <code>inverseOf</code> will be removed.
     * </p>
     * 
     * @param prop
     *            The property that this property is a inverse to.
     * @exception OntProfileException
     *                If the {@link Profile#INVERSE_OF()} property is not
     *                supported in the current language profile.
     */
    @Override
    ...
    /**
     * <p>
     * Answer a property that is an inverse of this property. If there is more
     * than one such property, an arbitrary selection is made.
     * </p>
     * 
     ...
     * <p>
     * Answer an iterator over all of the properties that are declared to be
     * inverse properties of this property. Each element of the iterator will be
     * an {@link OntProperty}.
     * </p>
     * 
     ...
     * @param prop
     *            A property to test for
     * @return True if the this property is the inverse of the the given
     *         property.
     */
    @Override
    ...
    /**
     * <p>
     * Remove the statement that this property is the inverse of the given
     * property. If this statement is not true of the current model, nothing
     * happens.
     * </p>
     * 
     * @param prop
     ...
     *                if the resource cannot be converted to a functional
     *                property given the language profile and the current state
     ...
     *                given the language profile and the current state of the
     *                underlying model.
     */
    @Override
     ...
     * @exception ConversionException
     *                if the resource cannot be converted to a transitive
     *                property given the language profile and the current state
     *                of the underlying model.
     */
    @Override
     ...
     * @exception ConversionException
     *                if the resource cannot be converted to an inverse
     *                functional property given the language profile and the
     ...
     * @exception ConversionException
     *                if the resource cannot be converted to a symmetric
     *                property given the language profile and the current state
     *                of the underlying model.
     */
    @Override
    ...
    /**
     * <p>
     * Answer a facet of this property as a functional property, adding
     * additional information to the model if necessary.
     * </p>
     * 
    ...
    public FunctionalProperty convertToFunctionalProperty() {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // return convertToType(getProfile().FUNCTIONAL_PROPERTY(),
        // "FUNCTIONAL_PROPERTY",
    ...
    /**
     * <p>
     * Answer a facet of this property as a datatype property, adding additional
     * information to the model if necessary.
     * </p>
     * 
    ...
    public DatatypeProperty convertToDatatypeProperty() {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // return convertToType(getProfile().DATATYPE_PROPERTY(),
        // "DATATYPE_PROPERTY",
    ...
    /**
     * <p>
     * Answer a facet of this property as an object property, adding additional
     * information to the model if necessary.
     * </p>
     * 
    ...
    public ObjectProperty convertToObjectProperty() {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // return convertToType(getProfile().OBJECT_PROPERTY(),
        // "OBJECT_PROPERTY",
    ...
    /**
     * <p>
     * Answer a facet of this property as a transitive property, adding
     * additional information to the model if necessary.
     * </p>
     * 
    ...
    public TransitiveProperty convertToTransitiveProperty() {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // return convertToType(getProfile().TRANSITIVE_PROPERTY(),
        // "TRANSITIVE_PROPERTY",
    ...
    /**
     * <p>
     * Answer a facet of this property as an inverse functional property, adding
     * additional information to the model if necessary.
     * </p>
     * 
     * @return This property, but converted to an InverseFunctionalProperty
     *         facet
     */
    @Override
    public InverseFunctionalProperty convertToInverseFunctionalProperty() {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // return convertToType(getProfile().INVERSE_FUNCTIONAL_PROPERTY(),
        // "INVERSE_FUNCTIONAL_PROPERTY", InverseFunctionalProperty.class);
    }

    /**
     * <p>
     * Answer a facet of this property as a symmetric property, adding
     * additional information to the model if necessary.
     * </p>
     * 
    ...
    public SymmetricProperty convertToSymmetricProperty() {
        throw new NotImplementedException("No implementation of our own has been created yet.");
        // return convertToType(getProfile().SYMMETRIC_PROPERTY(),
        // "SYMMETRIC_PROPERTY",
     ...
     * </p>
     * 
     * @return True if this this property has an <code>rdf:type</code> that
     *         defines it as a functional property.
     */
    @Override
     ...
     * 
     * @return True if this this property has an <code>rdf:type</code> that
     *         defines it as a datatype property.
     */
    @Override
     ...
     * 
     * @return True if this this property has an <code>rdf:type</code> that
     *         defines it as an object property.
     */
    @Override
     ...
     * 
     * @return True if this this property has an <code>rdf:type</code> that
     *         defines it as a transitive property.
     */
    @Override
     ...
     * 
     * @return True if this this property has an <code>rdf:type</code> that
     *         defines it as an inverse functional property.
     */
    @Override
    public boolean isInverseFunctionalProperty() {
        return hasRDFType(getProfile().INVERSE_FUNCTIONAL_PROPERTY(),
     ...
     * 
     * @return True if this this property has an <code>rdf:type</code> that
     *         defines it as a symmetric property.
     */
    @Override
    ...
    /**
     * <p>
     * Answer the property that is the inverse of this property. If no such
     * property is defined, return null. If more than one inverse is defined,
     * return an arbitrary selection.
     * </p>
     * 
    ...
    /**
     * <p>
     * Answer an iterator over the properties that are defined to be inverses of
     * this property.
     * </p>
     * 
     * @return An iterator over the properties that declare themselves the
     *         <code>inverseOf</code> this property.
     */
    @Override
    public ExtendedIterator<OntProperty> listInverse() {
        return getModel().listStatements(null, getProfile().INVERSE_OF(), this).mapWith(
                new SubjectAsMapper<>(OntProperty.class));
    }
    ...
    /**
     * <p>
     * Answer an iterator of all of the classes in this ontology, such that each
     * returned class has this property as one of its properties in
     * {@link OntClass#listDeclaredProperties()}. This simulates a frame-like
     * view of properties and classes; for more details see the <a
     * href="../../../../../../how-to/rdf-frames.html"> RDF frames howto</a>.
     * </p>
     * 
     * @return An iterator of the classes having this property as one of their
     *         declared properties
     */
    @Override
     ...
     * Answer an iterator of all of the classes in this ontology, such that each
     * returned class has this property as one of its properties in
     * {@link OntClass#listDeclaredProperties(boolean)}. This simulates a
     * frame-like view of properties and classes; for more details see the <a
     * href="../../../../../../how-to/rdf-frames.html"> RDF frames howto</a>.
     * </p>
     * 
     * @param direct
     *            If true, use only </em>direct</em> associations between
    ...
    public ExtendedIterator<OntClass> listDeclaringClasses(boolean direct) {
        ...
        // prop
        return WrappedIterator.create(cands.iterator()).filterKeep(
                new FilterDeclaringClass(this, direct));
    }

    /**
     * <p>
     * Answer an iterator over any restrictions that mention this property as
     * the property that the restriction is adding some constraint to. For
     * example:
     * </p>
     ...
     * <p>
     * <strong>Note</strong> that any such restrictions do not affect the global
     * semantics of this property itself. Restrictions define new class
     * expressions, and the property constraints are local to that class
     * expression. This method is provided as a convenience to assist callers to
     * navigate the relationships in the model.
     * </p>
     * 
    ...
    public ExtendedIterator<Restriction> listReferringRestrictions() {
        return getModel().listStatements(null, getProfile().ON_PROPERTY(), this).mapWith(
                new SubjectAsMapper<>(Restriction.class));
    }
     ...
     * model. If the given model is an ontology model, make the new property
     * object an ontproperty.
     * </p>
     * 
     * @param m
     *            A model
    ...
    /**
     * <p>
     * Filter that accepts classes which have the given property as one of their
     * declared properties.
     * </p>
     */
