...
/**
 * The SquareButton class is a simple SWT widget that can be used in place of a
 * native SWT button. Unlike the native button, this should look exactly the
 ...
 * web interface. You can very easily create your own styles that look
 * different. Colors can be specified for when the button is unselected,
 * inactive, focused, clicked, and hovered over. If two different background
 * colors are specified for a given button style, the button background will be
 * a top-to-bottom blended gradient of the two colors.
 * <p>
...
public class SquareButton extends Canvas {
    ...
    protected Font font;
    protected Color fontColor, hoverFontColor, clickedFontColor, inactiveFontColor,
            selectedFontColor;
    protected Color borderColor, hoverBorderColor, clickedBorderColor, inactiveBorderColor,
            selectedBorderColor;
    ...
    protected void addListeners() {
        ...
        this.addListener(SWT.MouseUp, new Listener() {
            public void handleEvent(Event e) {
                if (e.button == 1) {
                    SquareButton.this.setHoverColor(e);
                    if (enabled && (getClientArea().contains(e.x, e.y))) {
                        doButtonClicked();
                    }
                }
            }
    ...
    private void paintControl(PaintEvent e) {
        ...
        Point p = new Point(lastWidth, lastHeight);
        ...
        // such thing as a rounded gradient rectangle in SWT, so we need to draw
        // a filled rectangle that's just the right size to fit inside a rounded
        // rectangle without spilling out at the corners
        ...
        if (roundedCorners) {
            gc.drawRoundRectangle(rect.x + (bw - 1), rect.y + (bw - 1), rect.width - bw,
                    rect.height - 2, arcWidth, arcHeight);
        }
        else {
        ...
        if (this.isFocused && this.selectionBorder) {
            ...
            gc.setLineWidth(1);
            gc.drawRectangle(rect.x + (bw + 1), rect.y + (bw + 1), rect.width - (bw + 5),
                    rect.height - (bw + 5));
        }
    ...
    private int drawImage(GC gc, int x, int y) {
        ...
        Image imageToDraw = getEnabled() ? image
                : new Image(getDisplay(), image, SWT.IMAGE_DISABLE);
    ...
    private void drawBackgroundImage(GC gc, Rectangle rect) {
        ...
        if (backgroundImageStyle == BG_IMAGE_STRETCH) {
            gc.drawImage(backgroundImage, 0, 0, imgBounds.width, imgBounds.height, rect.x, rect.y,
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        if ((wHint == SWT.DEFAULT) && (hHint == SWT.DEFAULT) && !changed && (lastWidth > 0)
                && (lastHeight > 0)) {
