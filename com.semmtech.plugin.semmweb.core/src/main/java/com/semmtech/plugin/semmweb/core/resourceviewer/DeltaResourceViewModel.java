...
/**
 * Represents the difference, or delta, between two ResourceViewModel objects.
 * As some properties may be missing from this model compared to a full
 * ResourceViewModel (such as the text of each resource), the functions listed
 * in this class do not return the items in their correct order.
 */
public class DeltaResourceViewModel extends ModelCom {
    private Model deltaViewModel;
    private OntModel currentModel;

    public DeltaResourceViewModel(Model deltaViewModel, OntModel currentModel) {
